...
tempData$monthNum <- as.numeric(splitDate[,2])
tempData$dayNum <- as.numeric(splitDate[,3])
...
subset <- unique(seaData[,c("site", "year", "month")])
subset$count <- 1
sumFreq <- aggregate(subset["count"], subset[c("site", "year")], FUN = sum, na.rm = T); sumFreq
...
month <- aggregate(seaData["WaterTemp"], seaData[c("site", "year","month")], FUN = mean, na.rm = T)

write.csv(month, "m2m/output/monthlyTempLoggers.csv", row.names = F)
...
sites <- sort(unique(seaData$site))

seaData$dateNum <- as.numeric(paste(seaData$year, seaData$monthNum, seaData$dayNum, sep = ""))
seaData$dateType <- as.Date(seaData$date)

for(i in 1:length(sites)){
  temp <- subset(seaData, site == sites[i])
  temp$iter <- rep(1:nrow(temp))
