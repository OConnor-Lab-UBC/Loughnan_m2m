...
erddapLL$site[erddapLL$site == "goose"] <- "gooseSW"
erddapLL$site[erddapLL$site == "mcmullin"] <- "mcmullins"
...
compLL <- unique(merge(erddapLL, coord, by = "site"))
...
##########################################################################################
# compile the ERRDAP data:
colnames(erddapDat)[colnames(erddapDat) == "site"] <- "region"

# subset depth:
minDepth <- c(1.8, 2.8, 3.8, 4.8, 5.8)
maxDepth <- c(2.2, 3.2, 4.2, 5.2, 6.2)

erddap2m <- subset(erddapDat, depth> minDepth[2] & depth < maxDepth[2] )
# the ranges with the most data are 2 at 1441 rows data and 3m 1449, 

keepSttn <- c("KFPS04","KFPC01","KC12","KFPC06")
erddap2Sttn <- erddap2m[erddap2m$station %in% keepSttn, ]
erddap2Sttn$regionStn <- paste(erddap2Sttn$region, erddap2Sttn$station, sep = "_")
erddap2Sttn <- subset(erddap2Sttn, regionStn !="choked_KC12")

month <- aggregate(erddap2Sttn[c("temperature", "salinity", "dissolved_oxygen_ml_l", "dissolved_oxygen_percent")], erddap2Sttn[c("region", "year","month", "station")], FUN = mean, na.rm = T)

write.csv(month, "m2m/output/ctdMonthlyClim.csv", row.names = F)

# June-Aug
summer <- c("06", "07", "08")
summerT <- erddap2Sttn[erddap2Sttn$month %in% summer, ]
summerTemp <- aggregate(summerT[c("temperature", "salinity", "dissolved_oxygen_ml_l")], summerT[c("region", "year", "station")], FUN = mean, na.rm = T)
names(summerTemp)<- c("region" ,"year", "station", "sumTemp", "sumSalin", "sumDo")

# growing season: april-oct
growSea <- c( "04", "05", "06", "07", "08", "09")
aprilSept <- erddap2Sttn[erddap2Sttn$month %in% growSea, ]
growSeason <- aggregate(aprilSept[c("temperature", "salinity", "dissolved_oxygen_ml_l")], aprilSept[c("region", "year", "station")], FUN = mean, na.rm = T)
names(growSeason)<- c("region" ,"year", "station", "gsTemp", "gsSalin", "gsDo")

fin <- merge(summerTemp,growSeason, all = T, by = c("region","station", "year"))

write.csv(fin, "m2m/output/ctdSeasonalStn.csv", row.names = F)

##########################################################
# Macro site temperature data:
meay <- read.csv("m2m/data/Macro Sites Temperature 2014-2015/Meay_May_2014_Sept_2015.csv"); 
colnames(meay)[colnames(meay) == "AM_PM"] <- "am_pm"

choked <- meay
choked$region <- "choked"

pruth <- meay
pruth$region <- "pruth"

triquet <- read.csv("m2m/data/Macro Sites Temperature 2014-2015/Triquet_May_2014_Sept_2015.csv")
triquet$region <- "triquet"

goose <- read.csv("m2m/data/Macro Sites Temperature 2014-2015/Goose_Apr_Aug_2015.csv")
goose$region <- "goose"

mcmullins <- read.csv("m2m/data/Macro Sites Temperature 2014-2015/McMullins_Apr_Aug_2015.csv")
mcmullins$region <- "mcmullins"

macroTemp <- rbind(choked, pruth, triquet, goose, mcmullins)

temp <- str_split_fixed(macroTemp$Date, "-", 3)
macroTemp$month <- as.character(temp[,1])
macroTemp$day <- as.character(temp[,2])
macroTemp$year <- as.character(temp[,3])
macroTemp$year[which(macroTemp$year == "14")] <- "2014"
macroTemp$year[which(macroTemp$year == "15")] <- "2015"

# subset to only summer months:
summer <- c("06", "07", "08")

macroSum <- macroTemp[macroTemp$month %in% summer,]

macroMonth <- aggregate(macroSum[c("Temp")], macroSum[c("region","year")], FUN = mean, na.rm = T)

write.csv(macroMonth, "m2m/output/macroSeasonal.csv", row.names = F)
