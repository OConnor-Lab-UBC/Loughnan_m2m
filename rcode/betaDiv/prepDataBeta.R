...
m$site[which(m$site == "mcmullins_north")] <- "mcmullin_north"
m$site[which(m$site == "mcmullins_south")] <- "mcmullin_south"
...
# write.csv(abund, "m2m/output/cleanedGrazer.csv", row.names = F)
...
### Creating an object to store abundances only
m.comm <- abundG[,8:ncol(abundG)]

# - save the bray-curtis distances
for( i in 2014:2017 ){
  path <- "m2m/output/betaDiv/betaDivTemp"
  metai <- m.meta %>%
    filter(year==i) #%>%
...
############ meadow traitvs ############
sg <- read.csv("Data/R_Code_for_Data_Prep/master_data/MASTER_seagrass_metrics_20200214.csv")
sg <- sg[,2:ncol(sg)]

shoot <- read.csv("Data/R_Code_for_Data_Prep/master_data/MASTER_shoots.csv")
...
den2014 <- read.csv("m2m/data/Hakai2014/csvFiles/Shoot_density.csv")
...
bio2014 <- read.csv("m2m/data/Hakai2014/csvFiles/macro_plot_biomass_MHL.csv")
...
lai2014 <- read.csv("m2m/data/Hakai2014/csvFiles/Leaf_area.csv")
...
abio <- read.csv("Data/R_Code_for_Data_Prep/master_data/MASTER_abiotic_20200214.csv") # so little data not worth including further---depth only for 2016-2018, bed area only for 
...
# get depth data
depthActual <- read.csv("m2m/data/Hakai2014/csvFiles/Actual_depth_meso.csv")
...
seasonal <- read.csv("m2m/output/seasonalMatTemp.csv")
...
ctd <- read.csv("m2m/output/ctdSeasonalStn.csv")
ctd$region[which(ctd$region == "mcmullin")] <- "mcmullin"

macroClim <- read.csv("m2m/output/macroSeasonal.csv")
